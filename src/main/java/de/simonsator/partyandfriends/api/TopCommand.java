...
/**
 * Represents a TopCommand like /friend, /party or /command. The top command
 * decides if and which subcommand should be executed. Also if no subcommand
 * should be executed it outputs the help of the subcommands.
 *
 * @param <T>
...
public abstract class TopCommand<T extends SubCommand> extends Command implements Listener {
	...
	/**
	 * @param pCommandNames
	 *            The command name and the diffrent aliases of this command. By
	 *            these names the method can be called.
	 * @param pPermission
	 *            The permission which is needed to execute this command. If it
	 *            is blank no permission is needed to execute this command.
	 * @param pPrefix
	 *            The prefix which should be used by all subcommands. The prefix
	 *            gets returned by the method {@link #getPrefix()}.
	 */
	 ...
	 * @return Returns true if the sender of the command is a player. If not it
	 *         returns false and reloads the plugin.
	 */
	public static boolean isPlayer(CommandSender pCommandSender) {
		if (!(pCommandSender instanceof ProxiedPlayer)) {
			...
			pCommandSender.sendMessage(
					new TextComponent(Main.getInstance().getFriendsPrefix() + "Party and Friends was reloaded. "
	 ...
	 * {@link #isPlayer(CommandSender)}. If so it executes the method
	 * {@link #onCommand(OnlinePAFPlayer, String[])}. If not it reloads the
	 * plugin.
	 *
	 * @param pCommandSender
	 *            The sender of the command.
	...
	private boolean isDisabledServer(ProxiedPlayer pPlayer) {
		if (Main.getInstance().getConfig().getStringList("General.DisabledServers")
				.contains(pPlayer.getServer().getInfo().getName())) {
			pPlayer.sendMessage(
					new TextComponent(Main.getInstance().getMessagesYml().getString("General.DisabledServer")));
			return true;
		}
		return false;
	}
	...
	/**
	 * Decides if a subcommand should be called and if which one. If no
	 * subcommand should be called it shows the help of the commands.
	 *
	 * @param pPlayer
	...
	/**
	 * Adds a subcommand to the {@link #subCommands} list and after the
	 * subcommand was added it sorts the list again.
	 *
	 * @param pCommand
	...
	/**
	 * Sorts the subcommands by there priority. The subcommand with the lowest
	 * priority will be the first one in the list and the one with the highest
	 * priority the last one after the list was sorted.
	 */
	protected void sort() {
	...
	/**
	 * @param pClass
	 *            The type of the subcommand object which is searched
	 * @return Returns a subcommand of the {@link #subCommands} list, which has
	 *         an equal type.
	 */
